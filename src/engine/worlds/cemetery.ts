/**
 * STUB: the The Cemetery world. A subagent replaces this file wholesale; only the
...
function stubLevel(stage: number, runSeed: number, data: WorldData['data'] | null): LevelData {
  ...
  return {
    ...
    kind: 'world',
    theme: 'crypt',
    world: { kind: 'angels', stage, data: data ?? {}, won: false },
    width,
    height,
    tiles,
    start: { x: 1, y: 1 },
...
export const CEMETERY: WorldModule = {
  kind: 'angels',
  name: 'The Cemetery',
  intro: () => ({ title: 'The Cemetery', lines: ['This world is not built yet.'] }),

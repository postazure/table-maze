...
// recolored purple/magenta and topped with two devil horns rising off the
// bow — the "magic" counterpart to the plain gold chest key. Same
// rows/palette are reused for the HUD icon (icons.tsx) so the two read as a
// matched pair, just bigger and horned.
const DOOR_KEY_BODY = '#b56cff';
const DOOR_KEY_DARK = '#5a2596';
const DOOR_KEY_HORN = '#ff5c8a';
const DOOR_KEY_HORN_TIP = '#ffd0dc';
...
// Closed door: 16 wide x 24 tall — an arched plank door with two big devil
// horns growing out of the top of the arch, so the horned silhouette reads
// even at a glance. Planks/seams/outline are wood-brown; the horns (and the
// sliver of outline that touches only the horns) are the same purple/pink
// as the door key; a purple ring lock with a pink dot sits in the middle,
// matching the key's bow. Rows/palette generated by dilating a hand-placed
// body+horn mask by 1px for the outline (see scratch notes in the PR) —
// kept as a literal here since it's drawn once into an offscreen canvas.
// prettier-ignore
...
// Open door: just the arch frame outline + the two horns (no planks, no
// lock) so the horned silhouette still reads once the door swings open.
// Drawn at reduced alpha by drawDoor.
// prettier-ignore
...
export class Renderer implements TileMapper {
  ...
  /**
   * The door sprite is 16 wide x 24 tall (1 tile x 1.5 tiles) so the horns
   * can grow out of the top of the arch and overlap the wall tile above —
   * anchored to the BOTTOM of the door's own tile so that overlap reads as
   * "growing up out of this doorway" rather than floating.
   */
  ...
  private drawKey(ctx: CanvasRenderingContext2D, k: KeyItem, t: number): void {
    ...
    // the chest key) so their bold horned silhouette reads at a glance.
